...
const Flashcard = (props) => {
  ...
  const handleDeleteCard = (e) => {
    e.stopPropagation();
    requests
      .delete_word(props._id)
      .then((res) => {
        if (res.success) {
          const newWords = [...props.words].filter(
  ...
  return (
    <div className="Flashcard" onClick={flipCard}>
      <div className={`Card ${isFlipped ? "Flipped" : ""}`}>
        ...
        <div className="Back">
          <span className={props.admin || "space_for_results"}>
            {props.definition}
          </span>
          {!props.admin && (
            <div style={{ display: "block" }}>
              <button
                onClick={(e) => handleResult(e, true)}
              ...
              <button
                onClick={(e) => handleResult(e, false)}
                className="result_btn"
                style={{ color: "red" }}
              >
                I did not get it
