  ...
  <div>
    {
        ...
        ||
        ((done && <div className='header'>You have no more words to review; You are permanently done!</div>)
        ||
        ( showWords.length > 0 && <div className='header'> Word Count: {showWords.length} </div> ) 
        || 
        <div className='header'>You are temporarily done; please come back later to review more words.</div>)
    }
    {
        props.user.admin && <button onClick={props.toggleMode} className='change_mode' id="admin_btn">Admin Mode</button>
    }
    {   
        currentWord!==null  &&
